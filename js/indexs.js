...
clearInterval(tre);
    ...
    audio.play();
    starts.style.backgroundPositionY='-159px'
...
//加载列表数量
    ...
    starts.addEventListener("click",function () {
        ...
        if (audio.paused){   //paused这个属性返回的是true fales
            ...
            audio.pause();
            starts.style. backgroundPositionY='-198px';
            clearInterval(tre);
        }
//下一曲
next.addEventListener('click',function () {
    ...
    var getLrcTime = function(i){
        // 获取歌词里的每句的时间
        var lrcTime = searchData[i].substr(1,8);//"01:15.80"
        // 分钟转数字可以去掉前面的0
    ...
    setInterval(function(){
        ...
        for(var i = 0; i < loveStory.length; i++){
            ...
            // console.log(lrcTime);
            var lrcWord = loveStory[i].substr(10,searchData[i].length);
            if(timeNow > lrcTime){
                console.log(lrcTime);
                console.log(lrcWord);
                searchData.splice(i,1);//删除显示过的文本，
